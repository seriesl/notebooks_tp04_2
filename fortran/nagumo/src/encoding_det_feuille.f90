...
! +++++calcul du detail => erreur d'approximation  
            ...
            maille%fils(l)%ptr%det(m, l) = maille%fils(l)%ptr%u(m) - u_tilde(m)
...
! .....detail total pour seuillage et elagage
        ...
        do l = 1, 2**ndim
          ...
          do m = 1,nvar
            det_loc = det_loc + maille%fils(l)%ptr%det(m, l)*maille%fils(l)%ptr%det(m, l)
!            det_loc = det_loc + maille%det(m, l)*maille%det(m, l)
!!            det_loc = det_loc + abs( maille%det(m, l) )
          enddo

        maille%fils(l)%ptr%detail = dsqrt(det_loc/dble(nvar))      
...
! .....valeur extreme du detail
        det_max = max( det_max, maille%fils(l)%ptr%detail )
